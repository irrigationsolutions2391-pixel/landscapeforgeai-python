...
from novaai2_jav import Novaai2Jav, AsyncNovaai2Jav, DefaultAioHttpClient
from novaai2_jav._utils import is_dict
...
logging.getLogger("novaai2_jav").setLevel(logging.DEBUG)
...
def client(request: FixtureRequest) -> Iterator[Novaai2Jav]:
    strict = getattr(request, "param", True)
    if not isinstance(strict, bool):
        raise TypeError(f"Unexpected fixture parameter type {type(strict)}, expected {bool}")

    with Novaai2Jav(base_url=base_url, api_key=api_key, _strict_response_validation=strict) as client:
        yield client


@pytest.fixture(scope="session")
async def async_client(request: FixtureRequest) -> AsyncIterator[AsyncNovaai2Jav]:
